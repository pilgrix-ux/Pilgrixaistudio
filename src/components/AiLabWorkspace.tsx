...
export function AiLabWorkspace(): JSX.Element {
  ...
  const addFiles = useCallback(async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      try { next.push(await mediaService.createLocalAsset(file)) } catch { /* failed uploads are not presented as successful */ }
    }
  ...
  const send = useCallback(async (text = prompt) => {
    ...
    setMessages((current) => [...current, { id: crypto.randomUUID(), role: 'user', text: trimmed || 'Uploaded media.' }])
    setPrompt('')
    try {
      const result = await editorService.createEditJob({ prompt: trimmed || 'Analyze and prepare the uploaded media.', mediaIds: currentAttachments.map((asset) => asset.id) })
      setMessages((current) => [...current, { id: crypto.randomUUID(), role: 'assistant', text: result.message || 'Your edit request was accepted. I will show the finished result here when rendering completes.' }])
    } catch (error) {
    ...
    <main className="min-h-screen bg-[#f8f7f4] text-[#111]">
      ...
      <section className="mx-auto flex min-h-[calc(100vh-64px)] max-w-5xl flex-col px-4 pb-32 pt-6">
        <div className="flex-1 space-y-4 overflow-y-auto">
          ...
          {attachments.map((asset) => <div key={asset.id} className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-black/5"><video className="max-h-[420px] w-full object-contain" controls src={asset.url} /><div className="flex items-center gap-2 px-4 py-3 text-sm"><FileVideo size={17} /> {asset.name}</div></div>)}
