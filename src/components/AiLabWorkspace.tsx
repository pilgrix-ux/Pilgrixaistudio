      ...
      <header className={`sticky top-0 z-40 border-b px-4 py-3 backdrop-blur-2xl transition-colors ${dark ? 'border-white/[.06] bg-[#0b0d12]/90' : 'border-black/[.045] bg-white/90'}`}>
        <div className="mx-auto grid max-w-5xl grid-cols-[1fr_auto_1fr] items-center">
          <button
            ...
            className={`group justify-self-start rounded-[18px] border p-2.5 shadow-[0_6px_22px_rgba(15,23,42,.07)] transition hover:-translate-y-0.5 active:scale-95 ${dark ? 'border-white/[.08] bg-[#151821]' : 'border-black/[.07] bg-white'}`}
          >
          ...
          <button
            ...
            aria-label="Open Me"
            className={`group justify-self-end rounded-[18px] border p-2.5 shadow-[0_6px_22px_rgba(15,23,42,.07)] transition hover:-translate-y-0.5 active:scale-95 ${dark ? 'border-white/[.08] bg-[#151821]' : 'border-black/[.07] bg-white'}`}
          >
      ...
      <section className="mx-auto flex min-h-[calc(100vh-61px)] max-w-5xl flex-col px-4 pb-60 pt-9">
        <div className="mb-7">
          ...
          <div className="flex items-end justify-between gap-3">
            <div>
              <h1 className="text-[34px] font-semibold leading-[.98] tracking-[-.06em] sm:text-[44px]">
                What are we creating<br className="sm:hidden" /> today?
              </h1>
              <p className={`mt-3 max-w-[310px] text-[14px] leading-6 ${muted}`}>
                Bring the footage. Bring the idea. Pilgrix does the rest.
              </p>
            </div>
            <button
              type="button"
              onClick={startNewChat}
              className={`flex shrink-0 items-center gap-1.5 rounded-full border px-4 py-2.5 text-sm font-medium shadow-[0_7px_25px_rgba(15,23,42,.07)] transition active:scale-95 ${card}`}
            >
              <Plus size={17} />
              New
            </button>
        ...
        {messages.length > 1 && (
          <div className="mb-5 space-y-3">
            {messages.slice(1).map((message) => (
              <article
                key={message.id}
                className={`max-w-[88%] rounded-[24px] px-5 py-4 ${message.role === 'user' ? 'ml-auto bg-[#101217] text-white shadow-[0_8px_24px_rgba(0,0,0,.18)]' : `${card} border shadow-[0_8px_28px_rgba(0,0,0,.045)]`}`}
              >
        ...
        {working && (
          <div className={`mb-5 flex items-center gap-2 rounded-full border px-4 py-3 text-xs ${card} ${muted}`}>
            <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#00c7ff]" />
              ...
              {starterPrompts.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`group w-[142px] shrink-0 rounded-[20px] border p-3 text-left transition hover:-translate-y-0.5 active:scale-[.98] ${card}`}
                  >
                ...
                <div className="mb-2 flex gap-2 overflow-x-auto pb-1">
                  {attachments.map((attachment) => (
                    <div key={attachment.id} className={`relative h-14 w-14 shrink-0 overflow-hidden rounded-[12px] ${dark ? 'bg-[#202632]' : 'bg-white'} shadow-sm ring-1 ring-black/[.06]`}>
                      ...
                      <button type="button" onClick={() => removeAttachment(attachment.id)} aria-label="Remove attachment" className="absolute right-0.5 top-0.5 rounded-full bg-black/65 p-0.5 text-white">
                        <X size={10} />
                      </button>
                    </div>
                  ))}
            ...
            <div className="relative flex items-center justify-between gap-2 px-0.5 pb-0.5 pt-2">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`flex h-12 w-12 items-center justify-center rounded-full border transition hover:border-[#9bdff4] hover:text-[#00aeea] hover:shadow-[0_0_28px_rgba(0,196,255,.20)] active:scale-95 ${dark ? 'border-white/[.09] bg-[#171d27]' : 'border-black/[.07] bg-white'}`}
                >
                ...
                <button
                  ...
                  className={`flex h-12 w-12 items-center justify-center rounded-full border transition hover:border-[#9bdff4] hover:text-[#00aeea] active:scale-95 ${dark ? 'border-white/[.09] bg-[#171d27]' : 'border-black/[.07] bg-white'}`}
                >
                  <span className="grid grid-cols-2 gap-[4px]">
                    {Array.from({ length: 4 }).map((_, index) => <span key={index} className="h-[5px] w-[5px] rounded-full bg-current" />)}
                  </span>
                </button>
              </div>

              <div className="flex items-center gap-2">
                <button type="button" aria-label="Voice input" className={`hidden h-12 w-12 items-center justify-center rounded-full border sm:flex ${dark ? 'border-white/[.09] bg-[#171d27]' : 'border-black/[.07] bg-white'}`}>
                  <Mic size={19} />
                </button>
                <button
                  type="button"
                  ...
                  onClick={() => void send()}
                  aria-label={hasInput ? 'Create edit' : 'Voice input'}
                  className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full text-white shadow-[0_8px_22px_rgba(15,23,42,.18)] transition active:scale-95 ${hasInput ? 'bg-[#101217] hover:bg-[#1688d4] hover:shadow-[0_0_30px_rgba(0,174,234,.32)]' : dark ? 'bg-white/12 text-white/45' : 'bg-[#101217]'}`}
                >
        ...
        <div className="fixed inset-0 z-[60] bg-black/45 backdrop-blur-sm" onClick={() => setAttachmentSheet(false)}>
          <div onClick={(event) => event.stopPropagation()} className={`absolute bottom-0 left-0 right-0 rounded-t-[30px] p-5 pb-7 shadow-[0_-20px_70px_rgba(0,0,0,.25)] ${dark ? 'bg-[#111318] text-white' : 'bg-[#f8fafc] text-[#111318]'}`}>
            ...
            <div className="mb-5 flex items-center justify-between">
              <div>
                <div className="text-lg font-semibold">Add to your creation</div>
                <div className={`mt-1 text-xs ${muted}`}>Bring in the media Pilgrix should work with.</div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                const Icon = item.icon
                return (
                  <button key={item.label} type="button" onClick={item.action} className={`flex min-h-[105px] flex-col items-center justify-center gap-3 rounded-[22px] border transition active:scale-[.98] ${dark ? 'border-white/[.08] bg-[#1a1e26]' : 'border-black/[.05] bg-white shadow-sm'}`}>
                    <span className="flex h-12 w-12 items-center justify-center rounded-[16px] bg-[#1688d4]/10 text-[#1688d4]"><Icon size={23} /></span>
      ...
      {panel !== 'none' && (
        <div className="fixed inset-0 z-50 bg-black/35 backdrop-blur-[3px]" onClick={() => setPanel('none')}>
          <aside
            ...
            className={`absolute top-0 h-full w-[88%] max-w-[370px] p-5 shadow-[20px_0_60px_rgba(0,0,0,.22)] ${panel === 'menu' || panel === 'search' ? 'left-0' : 'right-0'} ${dark ? 'bg-[#111318] text-white' : 'bg-[#f8fafc] text-[#101318]'}`}
          >
            <div className="mb-7 flex items-center justify-between">
              <div>
                <div className="text-[9px] font-bold tracking-[.3em] text-[#1688d4]">PILGRIX</div>
                <div className="mt-1 text-xl font-semibold">
                  {panel === 'menu' ? 'Workspace' : panel === 'account' ? 'Me' : panel === 'projects' ? 'Projects' : panel === 'search' ? 'Search' : 'Settings'}
                </div>
              </div>
            ...
            {panel === 'menu' && (
              <div className="space-y-2">
                {menuItems.map((item) => {
                  const Icon = item.icon
                  return (
                    <button key={item.label} type="button" onClick={item.action} className={`flex w-full items-center gap-3 rounded-2xl border px-4 py-4 text-left shadow-sm transition hover:-translate-y-0.5 ${dark ? 'border-white/[.07] bg-[#181c24]' : 'border-black/[.05] bg-white'}`}>
                      <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${dark ? 'bg-white/5' : 'bg-[#f1f5f7]'}`}><Icon size={17} /></span>
                      <span className="flex-1 text-sm font-medium">{item.label}</span>
                      <ChevronRight size={15} className="opacity-25" />
                    </button>
                  )
                })}
              </div>
            ...
            {panel === 'account' && (
              <div className="space-y-3">
                <div className="rounded-[25px] bg-gradient-to-br from-[#0f1722] via-[#124a6a] to-[#6f42c1] p-5 text-white shadow-[0_15px_45px_rgba(22,136,212,.20)]">
                  <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/10"><User size={20} /></div>
                  <div className="mt-4 text-lg font-semibold">Your workspace</div>
                  <p className="mt-1 text-sm leading-5 text-white/55">Your account, plan and creative preferences.</p>
                </div>
                <button type="button" onClick={() => setTheme(dark ? 'light' : 'dark')} className={`flex w-full items-center justify-between rounded-2xl border px-4 py-4 text-sm ${dark ? 'border-white/[.07] bg-[#181c24]' : 'border-black/[.05] bg-white shadow-sm'}`}>
                  <span className="flex items-center gap-3">{dark ? <Moon size={17} /> : <Sun size={17} />} Appearance</span>
                  <span className="text-xs font-medium text-[#1688d4]">{dark ? 'Dark' : 'Light'}</span>
                </button>
                {['Profile & preferences', 'Plan & usage', 'Notifications'].map((item) => (
                  <button key={item} type="button" className={`flex w-full items-center justify-between rounded-2xl border px-4 py-4 text-sm ${dark ? 'border-white/[.07] bg-[#181c24]' : 'border-black/[.05] bg-white shadow-sm'}`}>
                    <span>{item}</span><ChevronRight size={15} className="opacity-25" />
                  </button>
                ))}
              </div>
            )}
            ...
            {panel === 'projects' && (
              <div className="space-y-3">
                {recentCreations.map((creation) => (
                  <button key={creation.title} type="button" onClick={() => setPanel('none')} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left ${dark ? 'border-white/[.07] bg-[#181c24]' : 'border-black/[.05] bg-white shadow-sm'}`}>
                    <div className={`h-14 w-14 shrink-0 rounded-xl bg-gradient-to-br ${creation.className}`} />
                    <div className="min-w-0"><div className="truncate text-sm font-semibold">{creation.title}</div><div className={`mt-1 text-xs ${muted}`}>{creation.meta}</div></div>
                  </button>
                ))}
                <button type="button" onClick={startNewChat} className="mt-2 w-full rounded-full bg-[#101217] px-4 py-3 text-xs font-semibold text-white">New creation</button>
              </div>
            )}

            {panel === 'search' && (
              <div>
                <div className={`flex items-center gap-2 rounded-2xl border px-4 py-3 ${dark ? 'border-white/[.07] bg-[#181c24]' : 'border-black/[.05] bg-white'}`}>
                  <Search size={17} className="opacity-40" />
                  <input autoFocus value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search your creations" className={`w-full bg-transparent text-sm outline-none ${dark ? 'placeholder:text-white/25' : 'placeholder:text-black/30'}`} />
                </div>
                <p className={`mt-5 text-center text-xs ${muted}`}>{search ? `No saved creations matching “${search}” yet.` : 'Search your AI creations and sessions.'}</p>
              </div>
            )}
            ...
            {panel === 'settings' && (
              <div className="space-y-2">
                <button type="button" onClick={() => setTheme(dark ? 'light' : 'dark')} className={`flex w-full items-center justify-between rounded-2xl border px-4 py-4 text-sm ${dark ? 'border-white/[.07] bg-[#181c24]' : 'border-black/[.05] bg-white shadow-sm'}`}>
                  <span className="flex items-center gap-3">{dark ? <Moon size={17} /> : <Sun size={17} />} Theme</span>
                  <span className="text-xs font-medium text-[#1688d4]">{dark ? 'Dark mode' : 'Light mode'}</span>
                </button>
                {['Notifications', 'AI preferences', 'Storage'].map((item) => (
                  <button key={item} type="button" className={`flex w-full items-center justify-between rounded-2xl border px-4 py-4 text-sm ${dark ? 'border-white/[.07] bg-[#181c24]' : 'border-black/[.05] bg-white shadow-sm'}`}>
                    <span>{item}</span><ChevronRight size={15} className="opacity-25" />
                  </button>
                ))}
