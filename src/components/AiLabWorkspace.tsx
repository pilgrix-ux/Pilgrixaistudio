  ...
  const send = useCallback(async () => {
    ...
    try {
      const result = await editorService.requestInstruction({
        ...
        context: { attachments: currentAttachments.map(({ id, file }) => ({ id, filename: file.name, mimeType: file.type, size: file.size })) },
      })
    ...
    <main className="min-h-screen bg-[#f5f3ee] text-[#111]">
      <header className="sticky top-0 z-40 border-b border-black/[0.055] bg-white/90 px-4 py-3 backdrop-blur-2xl">
        <div className="mx-auto grid max-w-5xl grid-cols-[1fr_auto_1fr] items-center">
          <button type="button" onClick={() => setPanel('menu')} className="group justify-self-start rounded-[18px] border border-black/[0.07] bg-white p-2.5 shadow-[0_4px_18px_rgba(0,0,0,.06)] transition hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,.09)] active:scale-95" aria-label="Open workspace menu">
            <Menu size={21} strokeWidth={1.8} className="transition group-hover:rotate-1" />
          </button>
          <button type="button" onClick={() => setPanel('projects')} className="text-center">
            <div className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#1688d4]">PILGRIX</div>
            <div className="mt-0.5 text-[18px] font-semibold tracking-[-0.02em]">AI Lab</div>
          </button>
          <button type="button" onClick={() => setPanel('account')} className="group justify-self-end rounded-[18px] border border-black/[0.07] bg-white p-2.5 shadow-[0_4px_18px_rgba(0,0,0,.06)] transition hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,.09)] active:scale-95" aria-label="Open Me">
            <User size={21} strokeWidth={1.8} className="transition group-hover:scale-105" />
          </button>
        </div>
      </header>
      ...
      <section className="mx-auto flex min-h-[calc(100vh-64px)] max-w-5xl flex-col px-4 pb-40 pt-7">
        <div className="mb-5 flex items-center justify-between">
          <div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-black/35">Edit session</p><h2 className="mt-1 text-xl font-semibold tracking-[-0.025em]">{chatTitle}</h2></div>
          <button type="button" onClick={startNewChat} className="flex items-center gap-1.5 rounded-full border border-black/[0.08] bg-white px-3.5 py-2 text-xs font-medium shadow-[0_4px_18px_rgba(0,0,0,.05)] hover:shadow-md"><Plus size={14} /> New</button>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-black/[0.05] bg-[#f5f3ee]/92 p-3 backdrop-blur-2xl">
          <div className="mx-auto max-w-5xl">
            <div className="mb-2 flex gap-2 overflow-x-auto pb-1">{starterPrompts.map((item) => <button key={item} type="button" onClick={() => setPrompt(item)} className="flex shrink-0 items-center gap-1.5 rounded-full border border-black/[0.06] bg-white px-3.5 py-2.5 text-xs shadow-[0_4px_18px_rgba(0,0,0,.045)] hover:-translate-y-0.5"><Sparkles size={12} /> {item}</button>)}</div>
            <div className="relative flex items-end gap-2 rounded-[30px] border border-black/[0.08] bg-white p-2 shadow-[0_14px_45px_rgba(0,0,0,.10)] ring-1 ring-white/80">
              ...
              <button type="button" onClick={() => fileInput.current?.click()} className="m-0.5 rounded-[19px] p-3 text-black/60 transition hover:bg-black/[0.05] hover:text-black" aria-label="Attach media"><Paperclip size={20} strokeWidth={1.8} /></button>
              <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send() } }} rows={1} placeholder="Tell Pilgrix what you want to make…" className="max-h-32 min-h-12 flex-1 resize-none bg-transparent px-1 py-3 text-[15px] leading-6 outline-none placeholder:text-black/35" />
              <button type="button" disabled={working || (!prompt.trim() && attachments.length === 0)} onClick={() => void send()} className="m-0.5 flex h-12 w-12 items-center justify-center rounded-[19px] bg-[#111] text-white shadow-[0_7px_20px_rgba(0,0,0,.18)] transition hover:-translate-y-0.5 hover:bg-[#1688d4] disabled:translate-y-0 disabled:opacity-30" aria-label="Send"><ArrowUp size={20} strokeWidth={2.2} /></button>
            </div>
            <p className="mt-2 text-center text-[10px] tracking-wide text-black/30">Your media stays attached to this edit session.</p>
          </div>
        </div>
      </section>

      {panel !== 'none' && <div className="fixed inset-0 z-50 bg-black/25 backdrop-blur-[2px]" onClick={() => setPanel('none')}>
        <aside onClick={(event) => event.stopPropagation()} className={`absolute top-0 h-full w-[88%] max-w-[370px] bg-[#fbfaf7] p-5 shadow-[20px_0_60px_rgba(0,0,0,.18)] ${panel === 'menu' ? 'left-0' : 'right-0'}`}>
          <div className="mb-8 flex items-center justify-between"><div><div className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#1688d4]">PILGRIX</div><div className="mt-1 text-xl font-semibold">{panel === 'menu' ? 'Workspace' : panel === 'account' ? 'Me' : 'Projects'}</div></div><button type="button" onClick={() => setPanel('none')} className="rounded-full p-2 hover:bg-black/5" aria-label="Close"><X size={20} /></button></div>
          {panel === 'menu' && <div className="space-y-2">{[['New edit', Plus], ['Projects', FolderOpen], ['Search chats', Search], ['Settings', Settings]].map(([label, Icon]) => <button key={String(label)} type="button" onClick={() => { if (label === 'New edit') startNewChat(); else if (label === 'Projects') setPanel('projects') }} className="flex w-full items-center gap-3 rounded-2xl bg-white px-4 py-4 text-left shadow-sm ring-1 ring-black/[0.05] transition hover:-translate-y-0.5"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#f1f5f7]"><Icon size={17} /></span><span className="flex-1 text-sm font-medium">{String(label)}</span><ChevronRight size={16} className="text-black/25" /></button>)}</div>}
