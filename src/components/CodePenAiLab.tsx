    ...
    <main className="fixed inset-0 z-0 flex h-[100dvh] w-full flex-col overflow-hidden overscroll-none bg-slate-50/90 font-sans text-slate-800">
      ...
      {menuOpen && <div className="fixed inset-0 z-50 bg-slate-950/20 backdrop-blur-[3px]" onClick={() => setMenuOpen(false)}><aside className="relative flex h-[100dvh] w-[84vw] max-w-[390px] flex-col border-r border-white/70 bg-gradient-to-b from-white via-sky-50/95 to-indigo-50/90 shadow-[18px_0_55px_rgba(51,65,85,0.14)] backdrop-blur-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="min-h-0 flex-1 overflow-hidden px-4">
          ...
          <button type="button" className="group mb-5 flex w-full items-center justify-between rounded-2xl border border-white/80 bg-white/60 px-3.5 py-3.5 text-left shadow-sm shadow-slate-300/10 backdrop-blur-md transition hover:bg-white/85 active:bg-white"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-sky-100 to-indigo-100 text-indigo-500"><Images size={18} strokeWidth={1.9} /></span><div><span className="block text-sm font-semibold text-slate-700">Image studio</span><span className="mt-0.5 block text-[10px] font-medium text-slate-400">Create and edit visuals</span></div></div><ChevronRight size={17} className="text-slate-300 group-hover:text-indigo-500" /></button>
