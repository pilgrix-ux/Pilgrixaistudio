          ...
          <section onClick={(e) => e.stopPropagation()} className="w-full max-w-xl rounded-t-[32px] border border-white/90 bg-[#f7faff]/95 px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-3 shadow-[0_-20px_70px_rgba(15,23,42,0.18)] backdrop-blur-2xl animate-[slideUp_.22s_ease-out]">
            ...
            <div className="relative mt-4 max-h-[38vh] overflow-hidden rounded-2xl border border-white bg-white/70">
              ...
              <div onScroll={(e) => setPickerScrolled(e.currentTarget.scrollTop > 4)} className="max-h-[38vh] overflow-y-auto p-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {attachments.length > 0 ? (<><div className="mb-2 flex items-center justify-between"><span className="text-xs font-bold text-slate-700">Selected media</span><span className="text-[10px] font-semibold text-slate-400">{mediaLabel}</span></div><div className="grid grid-cols-3 gap-2">{attachments.map((item) => <div key={item.id} className="relative aspect-square overflow-hidden rounded-xl bg-slate-100">{item.kind === 'image' ? <img src={item.url} alt={item.file.name} className="h-full w-full object-cover" /> : item.kind === 'video' ? <video src={item.url} muted playsInline className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center"><File size={25} /></div>}<button type="button" onClick={() => removeAttachment(item.id)} className="absolute right-1.5 top-1.5 flex h-6 w-6 items-center justify-center rounded-full bg-black/65 text-white" aria-label={`Remove ${item.file.name}`}><X size={13} /></button></div>)}</div><div className="h-12" /></>) : <div className="min-h-[145px] px-5 py-7 text-center"><p className="text-xs font-semibold text-slate-500">Nothing selected yet</p><p className="mt-1 text-[10px] font-medium text-slate-400">Choose Camera, Gallery, or Files above.</p></div>}
              </div>
