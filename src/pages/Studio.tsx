...
import { AlertCircle, ArrowUp, Bell, CheckCircle2, ChevronRight, Clock3, Download, Eraser, Film, Folder, Image as ImageIcon, Link2, Menu, Mic, MoreHorizontal, Play, Plus, Scissors, Search, Settings, Sparkles, Trash2, User, Wand2, X } from 'lucide-react'
...
export function Studio(): JSX.Element {
  ...
  const attachmentCountLabel = useMemo(() => attachments.length === 0 ? 'Add media' : `${attachments.length} ${attachments.length === 1 ? 'file' : 'files'} ready`, [attachments.length])

  const addFiles = (files: File[]): void => {
    const next = files.filter((file) => /^(video|audio|image)\//.test(file.type))
    ...
    if (!previewUrl) { const firstVideo = mapped.find((item) => item.file.type.startsWith('video/')); if (firstVideo) setPreviewUrl(firstVideo.url) }
  }
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>): void => { addFiles(Array.from(event.target.files ?? [])); event.target.value = '' }
  const removeAttachment = (id: string): void => { setAttachments((current) => { const removed = current.find((item) => item.id === id); if (removed) URL.revokeObjectURL(removed.url); return current.filter((item) => item.id !== id) }); if (previewUrl === attachments.find((item) => item.id === id)?.url) setPreviewUrl(null) }
  ...
  return <div className="pilgrix-shell">
    ...
    <header className="topbar">
      <button className="mobile-menu-button" onClick={() => setShowMenu((v) => !v)} aria-label="Open workspace menu"><Menu size={22} /></button>
    ...
    <main className="studio-frame">
      {view === 'ai' && <section className="ai-lab-view">
        <div className="ai-heading"><div><p className="eyebrow">EDIT WITH AI</p><h1>What are we making<br /><span>today?</span></h1></div><button className="soft-button new-edit-button" onClick={() => { setMessages([{ id: `welcome-${Date.now()}`, role: 'ai', text: 'Tell me what you want to make. Upload your footage or describe the edit.', time: 'Now' }]); setAttachments([]); setPrompt('') }}><Plus size={17} /> New edit</button></div>
        <div className="ai-canvas"><div className="conversation-column">{messages.map((message) => <article key={message.id} className={`message ${message.role}`}><div className={`message-avatar ${message.role}`}>{message.role === 'ai' ? <Sparkles size={15} /> : <User size={15} />}</div><div className="message-body"><div className="message-meta"><strong>{message.role === 'ai' ? 'Pilgrix' : 'You'}</strong><span>{message.time}</span></div><p>{message.text}</p></div></article>)}{isWorking && <div className="working-row"><div className="message-avatar ai"><Sparkles size={15} /></div><div><strong>Pilgrix is working</strong><p>Preparing your edit.</p></div><span className="thinking-orb" /></div>}{previewUrl && hasVideo && <div className="preview-card"><div className="preview-topline"><div><span className="eyebrow">PREVIEW</span><strong>Your footage</strong></div><button className="icon-button small" onClick={() => setPreviewUrl(null)} aria-label="Close preview"><X size={16} /></button></div><div className="video-stage"><video ref={videoRef} src={previewUrl} onEnded={() => setIsPlaying(false)} playsInline /><button className="video-play" onClick={togglePreview} aria-label={isPlaying ? 'Pause' : 'Play'}>{isPlaying ? <span className="pause-bars" /> : <Play size={20} fill="currentColor" />}</button><div className="video-label">ORIGINAL FOOTAGE</div></div><div className="preview-actions"><button onClick={() => sendPrompt('Make this into the strongest possible edit.')}>Make this an edit</button><button onClick={() => sendPrompt('Find the strongest moments.')}>Find best parts</button><button onClick={() => setView('main')}>Open project</button></div></div>}</div><aside className="context-rail"><div className="rail-card now-card"><div className="rail-label"><span className="blue-pulse" /> EDIT SESSION</div><h3>{attachments.length ? 'Untitled edit' : 'Ready for your footage'}</h3><p>{attachments.length ? `${attachmentCountLabel} · Ready when you are.` : 'Drop footage into the composer and describe the result you want.'}</p><button onClick={renderPreview}><Play size={15} /> Preview</button></div><div className="rail-card"><div className="rail-heading"><span>AI tools</span><MoreHorizontal size={16} /></div><div className="tool-list">{toolCards.map((tool) => { const Icon = tool.icon; return <button key={tool.title} onClick={() => setPrompt(tool.title)}><span className="tool-icon"><Icon size={16} /></span><span><strong>{tool.title}</strong><small>{tool.detail}</small></span><ChevronRight size={15} /></button> })}</div></div></aside></div>
        <div className="composer-wrap">{attachments.length > 0 && <div className="attachment-strip">{attachments.map((attachment) => <div className="attachment" key={attachment.id}>{attachment.file.type.startsWith('video/') ? <Film size={15} /> : attachment.file.type.startsWith('image/') ? <ImageIcon size={15} /> : <Mic size={15} />}<span>{attachment.file.name}</span><button onClick={() => removeAttachment(attachment.id)} aria-label={`Remove ${attachment.file.name}`}><X size={13} /></button></div>)}</div>}<div className="composer"><button className="composer-plus" onClick={() => inputRef.current?.click()} aria-label="Add media"><Plus size={21} /></button><input ref={inputRef} type="file" accept="video/*,audio/*,image/*" multiple hidden onChange={handleFileChange} /><div className="composer-main"><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); sendPrompt() } }} placeholder="Tell Pilgrix what you want to create..." rows={1} /><div className="composer-mode"><Sparkles size={13} /><span>AI edit</span><span className="mode-dot" /><span>Use my footage</span></div></div><button className="composer-mic" onClick={() => setPrompt((current) => current ? `${current} ` : 'Create a natural, clean edit. ')} aria-label="Voice input"><Mic size={19} /></button><button className="send-button" onClick={() => sendPrompt()} aria-label="Create"><ArrowUp size={21} /></button></div><div className="composer-hint"><span><Plus size={13} /> Add footage, reference or image</span><span>MP4 · MOV · image · audio</span></div></div>
