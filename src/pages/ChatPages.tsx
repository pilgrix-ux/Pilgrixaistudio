...
function SectionTitle({ icon, title, count }: { icon: JSX.Element; title: string; count?: number }): JSX.Element {
  return <div className="mb-3 flex items-center justify-between px-1"><div className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-xl border border-white bg-white/75 text-sky-500 shadow-sm">{icon}</span><h2 className="text-sm font-black tracking-tight text-slate-800">{title}</h2>{typeof count === 'number' && count > 0 && <span className="rounded-full bg-sky-50 px-2 py-0.5 text-[9px] font-bold text-sky-500">{count}</span>}</div></div>
}
...
export function ChatHistoryPage({ onBack, onOpenChat }: HistoryProps): JSX.Element {
  ...
  const filtered = useMemo(() => {
    ...
    const searched = value ? chats.filter((chat) => `${chat.title} ${chat.preview}`.toLowerCase().includes(value)) : chats
    if (filter === 'images') return searched.filter((chat) => (chat.attachmentCount ?? 0) > 0)
    if (filter === 'videos') return []
    return searched
  }, [chats, query, filter])

  const chatItems = filter === 'images' || filter === 'videos' ? [] : filtered
  const imageItems = filter === 'chats' || filter === 'videos' ? [] : chats.filter((chat) => (chat.attachmentCount ?? 0) > 0).filter((chat) => !query.trim() || `${chat.title} ${chat.preview}`.toLowerCase().includes(query.trim().toLowerCase()))
  ...
  const emptyAll = chats.length === 0
  const emptyFiltered = filter !== 'videos' && filter !== 'images' && filtered.length === 0
  ...
  return (
    <main className="min-h-[100dvh] bg-[radial-gradient(circle_at_50%_-10%,rgba(186,230,253,.55),transparent_45%),#f7faff] px-4 pb-8 pt-[calc(1rem+env(safe-area-inset-top))] text-slate-900">
      <div className="mx-auto w-full max-w-xl">
        ...
        {emptyAll ? (
          <div className="flex min-h-[55vh] flex-col items-center justify-center px-6 text-center"><div className="relative flex h-20 w-20 items-center justify-center rounded-[28px] border border-white bg-white/75 text-sky-400 shadow-[0_20px_55px_rgba(56,189,248,.14)]"><MessageCircle size={30} strokeWidth={1.6} /><span className="absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full bg-cyan-300 text-white shadow-lg"><Sparkles size={12} /></span></div><h2 className="mt-6 text-xl font-black tracking-tight text-slate-800">No conversations or history yet</h2><p className="mt-2 max-w-xs text-sm leading-6 text-slate-400">Your conversations and creations will appear here after you start creating.</p></div>
        ) : filter === 'videos' ? (
          <div className="mt-12 rounded-[26px] border border-white/90 bg-white/65 p-7 text-center shadow-[0_12px_38px_rgba(30,64,175,.06)]"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-sky-50 text-sky-500"><Video size={24} /></div><h2 className="mt-4 text-base font-black text-slate-800">No video creations yet</h2><p className="mt-1 text-xs leading-5 text-slate-400">Your generated videos will have their own space here.</p></div>
        ) : filter === 'images' ? (
          <section className="mt-7"><SectionTitle icon={<ImageIcon size={15} />} title="Image creations" count={imageItems.length} />{imageItems.length === 0 ? <div className="rounded-[26px] border border-white/90 bg-white/65 p-7 text-center shadow-sm"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-500"><ImageIcon size={23} /></div><h2 className="mt-4 text-base font-black text-slate-800">No image creations yet</h2><p className="mt-1 text-xs leading-5 text-slate-400">Images you create will appear here as visual history.</p></div> : <div className="grid grid-cols-2 gap-3">{imageItems.map((chat) => <button key={chat.id} type="button" onClick={() => onOpenChat(chat.id)} className="overflow-hidden rounded-[22px] border border-white bg-white/75 text-left shadow-[0_12px_35px_rgba(30,64,175,.07)] transition active:scale-[.98]"><div className="flex h-28 items-center justify-center bg-gradient-to-br from-sky-50 via-white to-indigo-100 text-indigo-400"><ImageIcon size={30} strokeWidth={1.5} /></div><div className="p-3"><p className="truncate text-xs font-black text-slate-800">{chat.title}</p><p className="mt-1 text-[10px] font-semibold text-slate-400">{chat.attachmentCount} media · {formatDate(chat.createdAt)}</p></div></button>)}</div>}</section>
        ) : (
          <div className="mt-7 space-y-8">
            {(filter === 'all' || filter === 'chats') && <section><SectionTitle icon={<MessageCircle size={15} />} title="Conversations" count={chatItems.length} />{emptyFiltered ? <div className="rounded-[24px] border border-white/90 bg-white/65 p-6 text-center text-sm font-semibold text-slate-400">No conversations match your search.</div> : <div className="space-y-3">{chatItems.map((chat) => <div key={chat.id} className="group flex items-center gap-3 rounded-[24px] border border-white/90 bg-white/72 p-3 shadow-[0_12px_38px_rgba(30,64,175,.07)] backdrop-blur-xl transition hover:-translate-y-0.5"><button type="button" onClick={() => onOpenChat(chat.id)} className="flex min-w-0 flex-1 items-center gap-3 text-left active:scale-[.99]"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[18px] bg-gradient-to-br from-sky-100 via-white to-indigo-100 text-indigo-500 shadow-inner"><MessageCircle size={18} /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-extrabold text-slate-800">{chat.title}</span><span className="mt-1 block truncate text-xs font-medium text-slate-400">{chat.preview}</span></span><span className="shrink-0 text-[10px] font-bold text-slate-300">{formatDate(chat.createdAt)}</span><ChevronRight size={16} className="shrink-0 text-slate-300" /></button><button type="button" onClick={() => removeChat(chat.id)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-slate-300 transition hover:bg-rose-50 hover:text-rose-400" aria-label={`Delete ${chat.title}`}><Trash2 size={15} /></button></div>)}</div>}</section>}
            {(filter === 'all') && <section><SectionTitle icon={<ImageIcon size={15} />} title="Image creations" count={imageItems.length} />{imageItems.length === 0 ? <div className="rounded-[24px] border border-white/90 bg-white/60 p-5 text-center"><p className="text-xs font-bold text-slate-500">No image creations yet</p><p className="mt-1 text-[10px] font-medium text-slate-400">Generated images will appear here separately from chats.</p></div> : <div className="grid grid-cols-2 gap-3">{imageItems.map((chat) => <button key={chat.id} type="button" onClick={() => onOpenChat(chat.id)} className="overflow-hidden rounded-[22px] border border-white bg-white/75 text-left shadow-[0_12px_35px_rgba(30,64,175,.07)] active:scale-[.98]"><div className="flex h-28 items-center justify-center bg-gradient-to-br from-sky-50 via-white to-indigo-100 text-indigo-400"><ImageIcon size={30} strokeWidth={1.5} /></div><div className="p-3"><p className="truncate text-xs font-black text-slate-800">{chat.title}</p><p className="mt-1 text-[10px] font-semibold text-slate-400">{chat.attachmentCount} media · {formatDate(chat.createdAt)}</p></div></button>)}</div>}</section>}
            {filter === 'all' && <section><SectionTitle icon={<Video size={15} />} title="Video creations" /><div className="rounded-[24px] border border-white/90 bg-white/60 p-5 text-center"><p className="text-xs font-bold text-slate-500">No video creations yet</p><p className="mt-1 text-[10px] font-medium text-slate-400">Your generated videos will appear in their own section.</p></div></section>}
          </div>
        )}
      </div>
    </main>
