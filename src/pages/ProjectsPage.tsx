...
import { ArrowLeft, FolderOpen, Image, MoreVertical, Plus, Search, Trash2, Video } from 'lucide-react'
...
export function ProjectsPage({ onBack }: { onBack: () => void }): JSX.Element {
  ...
  const load = async (): Promise<void> => {
    ...
    if (!projectsResponse.ok) setError(projectsResponse.error?.userMessage ?? 'Unable to load projects.')
    else setProjects(projectsResponse.data ?? [])
