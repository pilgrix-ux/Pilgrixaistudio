        ...
        <div className="mx-auto flex min-h-full w-full max-w-xl flex-col">
          {messages.length === 0 ? (
            <div className="flex flex-1 flex-col items-center justify-center px-2 pb-8 pt-4 text-center">
              <div className="relative mb-5 flex h-24 w-24 items-center justify-center rounded-[30px] border border-white/90 bg-white/75 shadow-xl shadow-indigo-500/10 backdrop-blur-xl"><div className="absolute inset-2 rounded-[24px] bg-gradient-to-br from-sky-100 via-white to-indigo-100" /><ImagePlus className="relative text-indigo-500" size={34} strokeWidth={1.7} /><span className="absolute right-4 top-4 h-2.5 w-2.5 rounded-full bg-sky-400 shadow-sm shadow-sky-400/40" /></div>
              ...
              <div className="mt-7 flex max-w-sm flex-wrap justify-center gap-2">{STARTER_PROMPTS.map((item) => <button key={item} type="button" onClick={() => useStarter(item)} className="rounded-full border border-indigo-100/80 bg-white/85 px-3.5 py-2 text-[10px] font-semibold text-slate-600 shadow-sm backdrop-blur-md transition active:scale-95 hover:border-indigo-200 hover:text-indigo-600">{item}</button>)}</div>
            </div>
          ) : (
        ...
        <div className="relative mx-auto max-w-xl rounded-[30px] border border-white/90 bg-white/92 p-2.5 shadow-[0_16px_50px_rgba(67,56,202,0.12)] backdrop-blur-2xl">
          ...
          <div className="mt-1 flex items-center justify-between px-1">
            <div className="flex items-center gap-1.5">
              <button type="button" onClick={() => setAttachmentMenuOpen((open) => !open)} className={`flex h-10 w-10 items-center justify-center rounded-full transition active:scale-95 ${attachmentMenuOpen ? 'rotate-45 bg-slate-900 text-white' : 'bg-slate-100 text-slate-600'}`} aria-label="Add reference"><Plus size={19} /></button>
            ...
            <div className="flex items-center gap-1.5"><button type="button" className="flex h-10 w-10 items-center justify-center rounded-full text-slate-400" aria-label="Voice input"><Mic size={17} /></button><button type="button" onClick={send} disabled={(!prompt.trim() && attachments.length === 0) || working} className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-sky-400 to-indigo-600 text-white shadow-lg shadow-indigo-500/20 transition active:scale-95 disabled:opacity-35" aria-label="Send image prompt"><ArrowUp size={18} strokeWidth={2.6} /></button></div>
          </div>
        </div>
      </footer>
    </main>
