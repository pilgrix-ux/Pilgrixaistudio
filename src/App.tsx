...
import '@/styles/pilgrix-polish.css'
...
function App(): JSX.Element {
  ...
  useEffect(() => {
    ...
    window.addEventListener('hashchange', handleNavigationChange); window.addEventListener('popstate', handleNavigationChange); window.addEventListener('pilgrix-theme-change', handleThemeChange)
    let active = true
    ...
    return () => { active = false; stopConversationPersistence(); window.removeEventListener('hashchange', handleNavigationChange); window.removeEventListener('popstate', handleNavigationChange); window.removeEventListener('pilgrix-theme-change', handleThemeChange) }
  }, [])
  const navigate = (nextPage: Page, returnToMenu = false, chatId = ''): void => {
    ...
    if (window.location.hash !== nextHash) window.history.pushState(null, '', `${window.location.pathname}${window.location.search}${nextHash}`); setPage(nextPage)
  }
