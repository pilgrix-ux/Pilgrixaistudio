...
function triggerSource(source: 'camera' | 'gallery' | 'files'): void {
  ...
  // Camera opens the device camera. Gallery is intentionally limited to
  // media. Files is a true system file picker: no accept filter means the
  // user's normal Files/document picker can show PDFs, documents, archives,
  // audio, video, images, and other supported file types.
  if (source === 'camera') {
  ...
  sheet.innerHTML = `
    ...
    <section class="attachment-sheet-panel" role="dialog" aria-modal="true" aria-labelledby="attachment-sheet-title">
      ...
      <div class="attachment-sheet-header">
        <div><span class="attachment-sheet-eyebrow">ADD TO THIS EDIT</span><h2 id="attachment-sheet-title">Bring in your media</h2><p>Choose footage, images or files from your device.</p></div>
      ...
      <div class="attachment-source-grid">
        ...
        <button class="attachment-source" data-source="gallery"><span class="attachment-source-icon">${icon('gallery')}</span><strong>Photos</strong><small>Photos & videos</small></button>
        <button class="attachment-source" data-source="files"><span class="attachment-source-icon">${icon('files')}</span><strong>Files</strong><small>Open your device files</small></button>
      </div>
      <div class="attachment-recent-head"><div><span class="attachment-sheet-eyebrow">YOUR DEVICE</span><strong>Choose something to add</strong></div></div>
      <div class="attachment-recent-grid" aria-label="Device media options">
        <button class="attachment-recent-empty" data-source="gallery"><span>+</span><strong>Choose photos or videos</strong><small>Open your device gallery</small></button>
        <button class="attachment-recent-tip" data-source="files"><span>⌁</span><strong>Browse files</strong><small>Open the normal system Files picker</small></button>
      </div>
      <div class="attachment-sheet-footer"><span class="attachment-status-dot"></span><span>Nothing is added until you choose a real file.</span></div>
    </section>`
