...
import { createPersistentEntitlementService, isPersistentEntitlementConfigured } from './persistent-entitlement.mjs'
...
const signupRateLimit = { maxPerWindow: fromEnv('FREE_TRIAL_SIGNUP_RATE_LIMIT', 5), windowMs: fromEnv('FREE_TRIAL_SIGNUP_RATE_WINDOW_MS', 60_000) }; const videoRateLimit = { maxPerWindow: fromEnv('FREE_TRIAL_VIDEO_RATE_LIMIT', 20), windowMs: fromEnv('FREE_TRIAL_VIDEO_RATE_WINDOW_MS', 60_000) }
const sendJson = (res, statusCode, body) => { res.writeHead(statusCode, { 'Content-Type': 'application/json; charset=utf-8', 'Access-Control-Allow-Origin': process.env.CORS_ORIGIN || 'http://localhost:5173', 'Access-Control-Allow-Methods': 'GET,POST,PATCH,OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type,Authorization,X-Device-Id,X-Network-Id' }); res.end(JSON.stringify(body)) }
...
const authorizeEntitlement = async (auth) => entitlementService.authorize({ userId: auth.userId, authenticated: true, plan: getPlanFromClaims(auth.claims) })
const consumeEntitlement = async (auth, body) => entitlementService.consume({ userId: auth.userId, authenticated: true, plan: getPlanFromClaims(auth.claims), requestId: typeof body.requestId === 'string' ? body.requestId : undefined })
...
const server = http.createServer(async (req, res) => {
  ...
  const body = await parseBody(req); const auth = await authenticate(req); if (!auth.ok) return sendJson(res, auth.status, { ok: false, error: auth.error }); const identity = getIdentity(req, auth, body)
  ...
  if (url.pathname === '/api/security/otp/verify') { const code = typeof body.code === 'string' ? body.code.trim() : ''; const result = await securityGate.verifyOtpChallenge({ userId: auth.userId, code }); if (!result.ok) { await securityGate.markOtpFailure({ userId: auth.userId }); return sendJson(res, result.status || 401, { ok: false, error: result.error }) }; await securityGate.markOtpVerified({ userId: auth.userId }); return sendJson(res, 200, { ok: true, verified: true }) }
  ...
  const security = await securityGate.authorizeVideo({ ...identity, authenticated: true, plan: getPlanFromClaims(auth.claims) }); if (!security.ok) return sendJson(res, security.status, { ok: false, error: { code: security.action === 'verify_phone' ? 'phone_verification_required' : security.action === 'upgrade' ? 'entitlement_limit_exceeded' : 'security_blocked', userMessage: security.action === 'verify_phone' ? 'Please verify your phone number before continuing.' : security.action === 'upgrade' ? 'Your free trial has ended. Upgrade to keep creating.' : 'We could not approve this request. Please try again later.', status: security.status, retryable: false } })
  const entitlementAuth = await authorizeEntitlement(auth); if (!entitlementAuth.ok) return sendJson(res, entitlementAuth.status, { ok: false, error: entitlementAuth.error, entitlement: entitlementView(entitlementAuth) })
  await securityGate.rememberIdentity(identity)
  const result = await callProvider(prompt, typeof body.action === 'string' ? body.action : 'enhance', body.context && typeof body.context === 'object' ? body.context : {}); if (!result.ok) return sendJson(res, result.error.status || 500, { ok: false, error: result.error })
  const consumed = await consumeEntitlement(auth, body); if (!consumed.ok) return sendJson(res, consumed.status, { ok: false, error: consumed.error, entitlement: entitlementView(consumed) })
  await securityGate.consumeVideo({ ...identity, requestId: typeof body.requestId === 'string' ? body.requestId : undefined })
